        ...
        T: Deserialize<'de>,
    {
            ...
            D: Deserializer<'de>,
        {
            ...
            deserializer.deserialize_seq(visitor).map(|v| {
                BoundedVec::<T, S>::try_from(v).map_err(|_| Error::custom("out of bounds"))
            })?
        }
    }
}
    ...
    Bound: Get<u32>,
{
    ...
    fn try_collect(self) -> Result<BoundedVec<T, Bound>, Self::Error> {
        if self.len() > Bound::get() as usize {
            ...
            Ok(BoundedVec::<T, Bound>::unchecked_from(
                self.collect::<Vec<T>>(),
            ))
        }
    }
...
macro_rules! codec_impl {
    ($codec:ident) => {
        ...
        use $codec::{
            decode_vec_with_len, Compact, Decode, DecodeLength, DecodeWithMemTracking, Encode,
            EncodeLike, Error, Input, MaxEncodedLen,
        };
        ...
        impl<'a, T: Encode + Decode, S: Get<u32>> EncodeLike<BoundedVec<T, S>>
...
mod test {
    ...
    fn can_be_collected() {
        ...
        let b2: BoundedVec<u32, ConstU32<4>> = b1
            .iter()
            .map(|x| x + 1)
            .rev()
            .skip(2)
        ...
        let b2: BoundedVec<u32, ConstU32<2>> = b1
            .iter()
            .map(|x| x + 1)
            .rev()
            .skip(2)
    ...
    fn container_can_derive_hash() {
        ...
        let _foo = Foo {
            bar: 42,
            slice: BoundedSlice::truncate_from(&[0, 1][..]),
            map: BoundedVec::default(),
        };
    }
