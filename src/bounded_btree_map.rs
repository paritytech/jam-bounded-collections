    ...
    V: Deserialize<'de>,
{
        ...
        D: Deserializer<'de>,
    {
        ...
        deserializer.deserialize_map(visitor).map(|v| {
            BoundedBTreeMap::<K, V, S>::try_from(v).map_err(|_| {
                Error::custom("failed to create a BoundedBTreeMap from the provided map")
            })
        })?
    }
}
    ...
    Bound: Get<u32>,
{
    ...
    fn try_collect(self) -> Result<BoundedBTreeMap<K, V, Bound>, Self::Error> {
        if self.len() > Bound::get() as usize {
            ...
            Ok(BoundedBTreeMap::<K, V, Bound>::unchecked_from(
                self.collect::<BTreeMap<K, V>>(),
            ))
        }
    }
}
...
macro_rules! codec_impl {
    ($codec:ident) => {
        use super::*;
        use $codec::{
            Compact, Decode, DecodeLength, DecodeWithMemTracking, Encode, EncodeLike, Error, Input,
            MaxEncodedLen,
        };
...
mod test {
    ...
    fn encode_then_decode_gives_original_map() {
        let b = boundedmap_from_keys::<u32, ConstU32<7>>(&[1, 2, 3, 4, 5, 6]);
        let b_encode_decode =
            BoundedBTreeMap::<u32, (), ConstU32<7>>::decode(&mut &b.encode()[..]).unwrap();
    ...
    fn dont_consume_more_data_than_bounded_len() {
        ...
        assert_eq!(
            data_input.len(),
            data.len() - Compact::<u32>::compact_len(&(data.len() as u32))
        );
    }
    ...
    fn can_be_collected() {
        ...
        assert_eq!(
            b2.into_iter().map(|(k, _)| k).collect::<Vec<_>>(),
            vec![2, 3, 4, 5]
        );
        ...
        let b2: BoundedBTreeMap<u32, (), ConstU32<5>> = b1
            .iter()
            .map(|(k, v)| (k + 1, *v))
            .rev()
            .skip(2)
    ...
    #[test]
    fn test_iter_mut() {
        let mut b1: BoundedBTreeMap<u8, u8, ConstU32<7>> = [1, 2, 3, 4]
            .into_iter()
            .map(|k| (k, k))
            .try_collect()
            .unwrap();

        let b2: BoundedBTreeMap<u8, u8, ConstU32<7>> = [1, 2, 3, 4]
            .into_iter()
            .map(|k| (k, k * 2))
            .try_collect()
            .unwrap();
    ...
    fn map_maps_properly() {
        let b1: BoundedBTreeMap<u32, u32, ConstU32<7>> = [1, 2, 3, 4]
            .into_iter()
            .map(|k| (k, k * 2))
            .try_collect()
            .unwrap();
        let b2: BoundedBTreeMap<u32, u32, ConstU32<7>> = [1, 2, 3, 4]
            .into_iter()
            .map(|k| (k, k))
            .try_collect()
            .unwrap();
    ...
    fn try_map_retains_size() {
        ...
        assert_eq!(
            b1.len(),
            b2.try_map::<_, (), _>(|(_, _)| Ok(5_u32)).unwrap().len()
        );
    }
    ...
    fn try_map_short_circuit() {
        let b1: BoundedBTreeMap<u8, u8, ConstU32<7>> = [1, 2, 3, 4]
            .into_iter()
            .map(|k| (k, k))
            .try_collect()
            .unwrap();
    ...
    fn try_map_ok() {
        let b1: BoundedBTreeMap<u8, u8, ConstU32<7>> = [1, 2, 3, 4]
            ...
            .try_collect()
            .unwrap();
        let b2: BoundedBTreeMap<u8, u16, ConstU32<7>> = [1, 2, 3, 4]
            .into_iter()
            .map(|k| (k, (k as u16) * 100))
            .try_collect()
            .unwrap();
    ...
    fn prepend_compact_input_works() {
        ...
        let inner = [2, 3, 4];
        let mut input = PrependCompactInput {
            encoded_len: encoded_len.as_ref(),
            read: 0,
            inner: &mut &inner[..],
        };
    ...
    mod serde {
        ...
        fn test_bounded_btreemap_deserializer() {
            ...
            let map: Result<BoundedBTreeMap<u32, u32, ConstU32<6>>, serde_json::Error> =
        ...
        fn test_bounded_btreemap_deserializer_bound() {
            let json_str = r#"{"0":100,"1":101,"2":102}"#;
            let map: Result<BoundedBTreeMap<u32, u32, ConstU32<3>>, serde_json::Error> =
                serde_json::from_str(json_str);
            assert!(map.is_ok());
            let map = map.unwrap();
        ...
        fn test_bounded_btreemap_deserializer_failed() {
            ...
            let map: Result<BoundedBTreeMap<u32, u32, ConstU32<4>>, serde_json::Error> =
                serde_json::from_str(json_str);
