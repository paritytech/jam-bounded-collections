...
pub struct BoundedBTreeSet<T, S>(
    BTreeSet<T>,
    ...
    S: Clone,
{
        ...
        D: Deserializer<'de>,
    {
        ...
        deserializer.deserialize_seq(visitor).map(|v| {
            BoundedBTreeSet::<T, S>::try_from(v).map_err(|_| Error::custom("out of bounds"))
        })?
    }
}
    ...
    Bound: Get<u32>,
{
    ...
    fn try_collect(self) -> Result<BoundedBTreeSet<T, Bound>, Self::Error> {
        if self.len() > Bound::get() as usize {
            ...
            Ok(BoundedBTreeSet::<T, Bound>::unchecked_from(
                self.collect::<BTreeSet<T>>(),
            ))
        }
    }
}
...
macro_rules! codec_impl {
    ($codec:ident) => {
        use super::*;
        use $codec::{
            Compact, Decode, DecodeLength, Encode, EncodeLike, Error, Input, MaxEncodedLen,
        };
...
mod test {
    ...
    fn can_be_collected() {
        ...
        let b2: BoundedBTreeSet<u32, ConstU32<5>> = b1
            .iter()
            .map(|k| k + 1)
            .rev()
            .skip(2)
